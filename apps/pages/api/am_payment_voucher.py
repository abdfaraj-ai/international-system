"""
am_payment_voucher.py — API سند دفع
════════════════════════════════════
GET    /api/am/payment-voucher/        ← قائمة السندات
POST   /api/am/payment-voucher/        ← إنشاء سند جديد
GET    /api/am/payment-voucher/<id>/   ← تفاصيل سند
DELETE /api/am/payment-voucher/<id>/   ← حذف سند
"""
import json
...
def api_am_payment_voucher(request):
    ...
    if request.method == 'GET':
        ...
        return JsonResponse({
            'success':     True,
            'total':       total,
            'totalFrom':   float(qs.aggregate(t=Sum('from_amount'))['t'] or 0),
            'totalProfit': float(qs.aggregate(t=Sum('net_profit'))['t']  or 0),
            'page':        page,
    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            record = PaymentVoucher.objects.create(
        ...
        return JsonResponse({
            'success': True,
            'message': f'تم إنشاء سند الدفع {record.ref_number}',
...
def api_am_payment_voucher_detail(request, voucher_id):
    ...
    try:
        record = PaymentVoucher.objects.get(id=voucher_id)
